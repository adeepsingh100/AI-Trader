...
def test_detect_feature_importance_drift_flags_large_correlation_swing(monkeypatch):
    ...
    monkeypatch.setattr(models, "get_feature_importance", lambda mode, timeframe=None: rows)
...
def test_detect_feature_drift_no_crash_with_no_trades(monkeypatch):
    monkeypatch.setattr(models, "get_recently_closed_trades", lambda mode, since: [])
...
def test_detect_feature_drift_uses_entry_eval_features(monkeypatch):
    ...
    monkeypatch.setattr(models, "get_recently_closed_trades", lambda mode, since: baseline_trades + recent_trades)
...
def test_run_drift_detection_returns_summary_counts(monkeypatch):
    monkeypatch.setattr(models, "get_recently_closed_trades", lambda mode, since: [])
    monkeypatch.setattr(models, "get_feature_importance", lambda mode, timeframe=None: [])
