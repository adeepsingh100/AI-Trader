...
python3 -m src.agents.evolution_agent    # run hourly promotion-gate evaluation locally
...
3. **Opportunity Scorer** (`src/features/opportunity_scorer.py`) — deterministic weighted blend of 5 sub-scores (trend/momentum/volume/volatility/risk, `OPPORTUNITY_WEIGHT_*` in config) into one 0-100 score per symbol; also classifies market regime. `weighted_average()` is the public entry point other modules reuse. Clearing `MIN_OPPORTUNITY_SCORE`/`TOP_N_CANDIDATES` (entry) or dropping below `EXIT_SCORE_THRESHOLD` (exit) is necessary but not sufficient — see Risk Manager below for the Net Expectancy Gate an entry candidate must also clear. There is no LLM validation step asking anything for a second opinion (the module that used to sit here, `src/agents/signal_agent.py`, an LLM accept/reject gate, was removed entirely after a real incident where its provider's rate limit caused a multi-day trading outage — see below). Despite the name, the "risk" sub-score measures resistance headroom only (room to run before the next resistance level), not overall trade risk — see `_score_risk_for_timeframe`'s docstring; real risk (cost/liquidity, stop-distance, exposure, drawdown) is measured and gated independently in the Risk Manager and Portfolio Intelligence, not blended into this sub-score.
4. **Risk Manager** (`src/agents/risk_manager.py`) — position sizing, stop-loss/take-profit, circuit breaker, daily loss limits. Pure Python, no LLM. An entry candidate must also clear the **Net Expectancy Gate** (`compute_net_expectancy_pct`): fees/GST/TDS/spread/slippage netted against the resolved stop/target and the system's own calibrated win-probability estimate — `net_expectancy_pct <= 0` is itself the "no trade" decision. Stop-loss/take-profit (`resolve_exit_params`) prefer the evidence-validated `strategy_versions.params_json` value and fall back to an ATR-derived one only for a leg that isn't configured, so every position always has a defined risk boundary. Position sizing gets an additional cap derived from stop distance (`RISK_PER_TRADE_PCT` of capital per trade) on top of the existing flat/dynamic formula — it can only shrink size, never grow it.
5. **Execution Agent** (`src/agents/execution/`) — `ExecutionAgent` ABC, `PaperExecutionAgent` (simulated fills) and `RealExecutionAgent` (live CoinDCX orders — wired in but its order-placement path is unverified against a live fill; stays inert until a strategy version clears the promotion bar in PROJECT_SPEC.md §2).

An LLM (Groq, auto-falling back to Gemini — no provider-select switch, see `src/groq_client.py`) is still used, but only **hourly**, inside `AdaptiveStrategyEngine.analyze()` (`src/learning/recommendations.py::generate_ai_exit_params_recommendations`), offline from live trading entirely: it proposes a `stop_loss_pct`/`take_profit_pct` candidate, which still has to clear the same walk-forward/bootstrap-CI/fitness-floor gate as every other candidate before it can matter (see Learning system below). A failed or unavailable LLM call there just skips that one hourly proposal — it never blocks a trade.
...
**Invariant, narrowed**: nothing in `src/learning/` ever auto-writes to `config.py` or live scoring weights (`OPPORTUNITY_WEIGHT_*` and similar `os.getenv()` constants) — weight/regime/symbol-threshold recommendations stay advisory, a human still reviews `recommendations`/`adaptive_strategy_versions` rows in Supabase's table editor and manually copies values into env vars, exactly as before. What's no longer true as a blanket statement: **exit-params candidates** (`stop_loss_pct`/`take_profit_pct`, which target a DB row — `strategy_versions.params_json` — not an env var) **auto-activate** into a new `strategy_versions` row the moment they clear every statistical gate (`simulation.py::_activate_exit_params_candidate`), and `evolution_agent.py::run_evolution()` auto-flips `promoted_to_real` the same way once `src/learning/promotion_gate.py::evaluate_promotion()` returns `PROMOTE` — see PROJECT_SPEC.md §2/§3e for the full reasoning (auto-promotion itself isn't new; what changed is the bar — sample-size floors, risk/statistical/Monte-Carlo gates, regime/symbol robustness, and a significant same-market-data improvement over the current real-mode champion, not 5 simple thresholds; missing evidence always yields `EXTEND_VALIDATION`, never a silent promotion). Every evaluation (PROMOTE/REJECT/EXTEND_VALIDATION alike) is written to `promotion_audit`, and a promoted real-mode champion that later degrades gets **Automatic Rollback** — `strategy_health.py`'s existing auto-suspend, extended to write an explicit rollback record when the suspended version is the live champion (the fallback to the next-most-recent promoted version already happened structurally; this just audits it). The confidence-modifier chain in `confidence_calibration.py` (regime/symbol/recent-performance modifiers) remains the third automatic piece, extending the already-automatic, already-inert-by-default gate (`MIN_FINAL_CONFIDENCE=0`). Don't treat any of these three as precedent for auto-applying weight/regime/symbol-threshold recommendations too — that still needs env-var rewrites + a redeploy, a deliberately separate, bigger change.
...
**Data retention**: `models.purge_old_data()`, called every hour from `evolution_agent.run_evolution()` (no separate cron), deletes rows past a cutoff from the highest-write-volume, non-permanent tables (`opportunity_evaluations`, `confidence_calibration`, `agent_logs`, `model_usage`, `system_metrics`, `data_quality_log`) — keeps the free-tier Supabase disk from filling up the way it did once already. `trades`/`strategy_versions`/`recommendations`/`adaptive_strategy_versions`/`strategy_simulations`/`learning_statistics`/`feature_importance`/`drift_alerts`/`strategy_health_scores`/`promotion_audit`/`historical_candles` are never purged.
