...
4. **Signal Agent** (`src/agents/signal_agent.py`) — LLM (Groq, auto-falling back to Gemini if Groq's whole chain fails — no provider-select switch, see `src/groq_client.py`) validates/rejects the quant candidates and returns structured reasoning; never invents candidates itself.
